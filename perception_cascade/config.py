...
Point it at ANY directory tree of timestamped image frames
(organized as <captures>/<group>/*.png with optional <name>.json
sidecars). Everything tunable lives here — no magic numbers in loops.
"""
...
WORKSPACE = Path(os.environ.get(
    "CASCADE_WORKSPACE",
    "~/.perception-cascade/workspace",
)).expanduser()
CAPTURES = Path(os.environ.get(
    "CASCADE_CAPTURES",
    str(WORKSPACE / "captures"),
)).expanduser()
OUT = Path(os.environ.get("CASCADE_OUT", str(WORKSPACE / "cascade_out"))).expanduser()
...
def load_prompt(env_var: str, default: str) -> str:
    """Domain prompts are config, not code. Point env_var at a text file
    to re-skin the loops for any domain (marine, agriculture, security
    cams, gauges) without touching source."""
